...
* **Heap-only, VarHandle-based typed access:** Despite its name suggesting low-level memory manipulation, modern `Slice` wraps only heap `byte[]` arrays — the historical `sun.misc.Unsafe` off-heap path was removed. All typed reads/writes go through `VarHandle` static handles that the JIT compiles to single machine instructions (e.g., `MOV` on x86), achieving near-native performance while remaining within the managed heap.
* **Zero-copy slicing with accurate retained-size accounting:** `slice(index, length)` creates a new `Slice` sharing the same backing `byte[]` with an adjusted `baseOffset`, but critically inherits the parent's `retainedSize`. This means sub-views correctly report the full array's memory cost to Trino's memory tracking hierarchy, preventing under-counting that would break the pool's accounting.
* **Dual growth strategy in `ensureSize`:** Below 512KB, capacity doubles; above 512KB, it grows by only 1.25x. This prevents runaway over-allocation for large buffers while keeping small buffers efficient. The implementation uses `Arrays.copyOfRange` instead of allocate-then-copy because the JVM optimizes `copyOfRange` to skip zeroing the new memory.
* **SWAR byte search instead of naive scanning:** `indexOfByte()` uses SIMD-Within-A-Register — packing the search byte into all 8 positions of a `long` and using the classic zero-byte detection bitmask trick `(xor - 0x0101...01) & ~xor & 0x8080...80` — to process 8 bytes per iteration instead of 1. This is a deliberate choice over JDK intrinsics, giving consistent cross-platform performance.
* **Benign race on cached hash:** The `hash` field is not `volatile`, so two threads may both compute XxHash64 and write the result. This is safe because `int` writes are atomic on the JVM and XxHash64 is deterministic — a deliberate "benign data race" pattern that avoids synchronization overhead on the hot read path.
